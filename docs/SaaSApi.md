...
> ApiV3SaasGet(ctx).Limit(limit).Skip(skip).Sortby(sortby).Filter(filter).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.SaaSApi.ApiV3SaasGet(context.Background()).Limit(limit).Skip(skip).Sortby(sortby).Filter(filter).Execute()
    if err != nil {
